...
def set_state(k, v):
    get_db()["state"].update_one({"key":k},{"$set":{"value":str(v)}},upsert=True)
...
def save_text(book_id, content, source="ocr"):
    if not content:
        return
    db = get_db()
    db["book_text"].update_one({"book_id":book_id},
        {"$set":{"book_id":book_id,"content":content,"source":source}},upsert=True)
